...
export default function PatientOverview() {
  ...
  const totalPaid = appointments.reduce(
    (sum, a) =>
      sum + (a.paymentStatus === "Paid" ? parseFloat(a.amount || 0) : 0),
    0,
  );
  ...
  return (
    <div className={`space-y-8 ${bgMain} transition-colors duration-300`}>
      ...
      <div>
        ...
        >
          Welcome, {session?.user?.name || "Patient"}. Here is your healthcare
          summary.
        </p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <div
                  className={`w-9 h-9 rounded-lg ${colors.bg} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div
          className={`${bgCard} border ${cardBorder} rounded-xl lg:col-span-2 transition-colors duration-300`}
        >
          <div
            className={`flex items-center justify-between p-5 border-b ${divider} transition-colors duration-300`}
          >
            <div>
              <h3
          ...
          <div className={`divide-y ${divLight}`}>
            {upcoming.length === 0 ? (
              <div className="p-10 text-center">
                <div
                  className={`w-12 h-12 mx-auto rounded-full ${statsColors.blue.bg} flex items-center justify-center mb-3 transition-colors duration-300`}
                >
              ...
              upcoming.slice(0, 5).map((a) => (
                <div key={a._id} className="p-5 flex items-center gap-4">
                  <div
                    className={`w-11 h-11 rounded-lg ${theme === "dark" ? "bg-slate-800" : "bg-gray-100"} border ${cardBorder} overflow-hidden flex-shrink-0 flex items-center justify-center transition-colors duration-300`}
                  >
                    {a.doctorPhoto ? (
                      <img
                        src={a.doctorPhoto}
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 ${theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700"} border rounded-full px-2.5 py-1 text-xs whitespace-nowrap transition-colors duration-300`}
                  >
        ...
        >
          <div>
            <div
              className={`w-10 h-10 rounded-lg ${statsColors.blue.bg} flex items-center justify-center mb-4 transition-colors duration-300`}
            >
              <Stethoscope className={`w-5 h-5 ${statsColors.blue.text}`} />
            </div>
            <h3
            ...
            <p
              className={`text-xs ${textSecondary} mt-2 leading-relaxed transition-colors duration-300`}
            >
              Quickly check available verified doctors to book your next
              consultation. We partner with only the finest clinics and
