...
export default function PatientAppointments() {
  ...
  const modalBg = theme === "dark" ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200";
  ...
  return (
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      {/* List */}
      {filtered.length === 0 ? (
        <div className={`${emptyBg} border ${cardBorder} rounded-xl p-12 text-center transition-colors duration-300`}>
          <Calendar className={`w-8 h-8 mx-auto ${textMuted} mb-3`} />
          <p className={`text-sm font-semibold ${textPrimary}`}>No appointments found</p>
        ...
        <div className={`${bgCard} border ${cardBorder} rounded-xl divide-y ${divLight} transition-colors duration-300`}>
          {filtered.map((a) => (
            <div key={a._id} className="p-5">
              <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
                <div className="flex items-start gap-4">
                  <div className={`w-11 h-11 rounded-lg ${avatarBg} overflow-hidden flex-shrink-0 flex items-center justify-center transition-colors duration-300`}>
                    {a.doctorPhoto ? (
                      <img src={a.doctorPhoto} alt="" className="w-full h-full object-cover" />
                    ) : (
              ...
              {a.symptoms && (
                <p className={`mt-3 pt-3 border-t ${divLight} text-xs ${textSecondary} transition-colors duration-300`}>
      ...
      {rescheduling && (
        ...
        >
          ...
          >
            <h3 className={`text-base font-semibold ${textPrimary}`}>Reschedule Consultation</h3>
            <p className={`text-xs ${textSecondary} mt-1`}>With {rescheduling.doctorName}</p>
            <form onSubmit={handleReschedule} className="mt-4 space-y-4">
              {!rescheduleDoctor ? (
                <div className="flex justify-center py-4">
                ...
                <>
                  <div>
                    <label className={`block text-xs font-medium ${textSecondary} mb-1.5 transition-colors duration-300`}>
                      Select New Date
                    </label>
                    <select
                      value={newDate}
...
function PaymentPill({ status, theme }) {
  const normalized = (status || "").toLowerCase();
  const bg = theme === "dark" ? "bg-slate-800 border-slate-700" : "bg-white border-gray-200";
  const text = theme === "dark" ? "text-slate-300" : "text-gray-700";
  
  if (normalized === "paid") {
    return (
      <span className={`inline-flex items-center gap-1 ${bg} border rounded-full px-2.5 py-1 text-xs ${text} whitespace-nowrap transition-colors duration-300`}>
