    ...
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      {reviews.length === 0 ? (
        <div className={`${bgCard} border ${cardBorder} rounded-xl p-12 text-center transition-colors duration-300`}>
          <Star className={`w-8 h-8 mx-auto ${emptyIcon} mb-3 transition-colors duration-300`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {reviews.map((r) => (
            <div key={r._id} className={`${bgCard} border ${cardBorder} rounded-xl p-5 flex flex-col justify-between transition-colors duration-300`}>
              <div>
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                  ...
                  <span className={`text-[10px] ${textMuted} whitespace-nowrap transition-colors duration-300`}>
                    {r.createdAt ? new Date(r.createdAt).toLocaleDateString() : "—"}
                  </span>
                </div>
                {r.reviewText && (
                  <p className={`mt-3 text-sm ${reviewText} leading-relaxed italic transition-colors duration-300`}>
                    "{r.reviewText}"
                  </p>
                )}
              </div>

              <div className={`mt-4 pt-3 border-t ${divLight} flex items-center justify-end gap-1.5 transition-colors duration-300`}>
                <button
                  onClick={() => setEditingReview(r)}
...
function ReviewModal({ review, visitedDoctors, onClose, onSaved, patientId, patientName, theme }) {
  const [rating, setRating] = useState(review?.rating || 5);
  const [reviewText, setReviewText] = useState(review?.reviewText || "");
  const [doctorId, setDoctorId] = useState(review?.doctorId || visitedDoctors[0]?.doctorId || "");
