...
//           router.push(`/dashboard/doctor/prescriptions?patientId=${app.patientId}&appointmentId=${app._id}`);
//         } else {
//           fetchRequests();
//         }
//       } else {
...
//       {/* Title */}
//       <div>
//         <h1 className="text-2xl font-bold tracking-tight text-gray-900">Appointment Requests</h1>
//         <p className="text-sm text-gray-500 mt-1">Review patient consultation queries and update their status.</p>
//       </div>
...
//                   <div className="w-10 h-10 rounded-full bg-gray-100 border border-gray-200 flex items-center justify-center text-gray-400 text-sm font-semibold flex-shrink-0">
//                     {(a.patientName || "P")[0]}
//                   </div>
//                   <div className="flex-1 min-w-0">
//                     <p className="text-sm font-semibold text-gray-900">{a.patientName}</p>
//                     <p className="text-xs text-gray-500 mt-0.5">{a.patientEmail || "N/A"}</p>
...
//                         className="inline-flex items-center gap-1 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors"
//                       >
//                         <Check className="w-3.5 h-3.5" /> Accept
//                       </button>
//                       <button
//                         onClick={() => updateStatus(a._id, "Rejected")}
//                         className="inline-flex items-center gap-1 bg-white hover:bg-red-50 border border-gray-200 hover:border-red-200 text-red-600 rounded-lg px-2.5 py-1.5 text-xs font-medium transition-colors"
//                       >
...
//                       className="inline-flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white rounded-lg px-3 py-1.5 text-xs font-medium transition-colors"
//                     >
//                       Mark completed →
//                     </button>
//                   )}
...
//                     <button
//                       onClick={() => router.push(`/dashboard/doctor/prescriptions?patientId=${a.patientId}&appointmentId=${a._id}`)}
//                       className="inline-flex items-center gap-1 bg-white border border-gray-200 hover:bg-gray-50 rounded-lg px-2.5 py-1.5 text-xs font-medium text-gray-700 transition-colors"
//                     >
...
export default function DoctorRequests() {
  ...
  const tabActive = theme === "dark" ? "text-white border-blue-400" : "text-gray-900 border-blue-600";
  const tabInactive = theme === "dark" ? "text-slate-400 hover:text-slate-300" : "text-gray-500 hover:text-gray-700";
  const emptyIcon = theme === "dark" ? "text-slate-600" : "text-gray-300";
  const avatarBg = theme === "dark" ? "bg-slate-800 border-slate-700" : "bg-gray-100 border-gray-200";
  ...
  const acceptBtn = theme === "dark" ? "bg-blue-950/30 hover:bg-blue-950/50 text-blue-400" : "bg-blue-50 hover:bg-blue-100 text-blue-600";
  const rejectBtn = theme === "dark" ? "bg-slate-800 hover:bg-red-950/30 border-slate-700 hover:border-red-800 text-red-400" : "bg-white hover:bg-red-50 border-gray-200 hover:border-red-200 text-red-600";
  const completeBtn = theme === "dark" ? "bg-blue-600 hover:bg-blue-700 text-white" : "bg-blue-600 hover:bg-blue-700 text-white";
  const prescriptionBtn = theme === "dark" ? "bg-slate-800 border-slate-700 hover:bg-slate-700 text-slate-300" : "bg-white border-gray-200 hover:bg-gray-50 text-gray-700";
  const statusPillBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  ...
  return (
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      {/* Tabs */}
      <div className={`border-b ${divider} flex items-center gap-1 overflow-x-auto whitespace-nowrap transition-colors duration-300`}>
        {[
          ["all", "All"],
          ["pending", "Pending"],
          ["accepted", "Accepted"],
      ...
      {/* List */}
      {filtered.length === 0 ? (
        <div className={`${bgCard} border ${cardBorder} rounded-xl p-12 text-center transition-colors duration-300`}>
          <Inbox className={`w-8 h-8 mx-auto ${emptyIcon} mb-3 transition-colors duration-300`} />
        ...
        <div className={`${bgCard} border ${cardBorder} rounded-xl divide-y ${divLight} transition-colors duration-300`}>
          {filtered.map((a) => (
            <div key={a._id} className="p-5">
              <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-full ${avatarBg} flex items-center justify-center text-sm font-semibold flex-shrink-0 ${avatarText} transition-colors duration-300`}>
                    {(a.patientName || "P")[0]}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold ${textPrimary} transition-colors duration-300`}>
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2.5">
                      <span className={`inline-flex items-center gap-1 ${tagBg} border rounded-full px-2.5 py-1 text-xs transition-colors duration-300`}>
              ...
              {a.symptoms && (
                <p className={`mt-3 pt-3 border-t ${divLight} text-xs ${symptomText} transition-colors duration-300`}>
                  <span className={`${symptomLabel} font-medium transition-colors duration-300`}>Symptoms: </span>
                  {a.symptoms}
                </p>
              )}
            </div>
...
function StatusPill({ status, theme }) {
  ...
  const s = map[normalized] || { dot: "bg-gray-400", label: status };
  const bg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  
  return (
    <span className={`inline-flex items-center gap-1.5 ${bg} border rounded-full px-2.5 py-1 text-xs whitespace-nowrap transition-colors duration-300`}>
      <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
      {s.label}
    </span>
  );
