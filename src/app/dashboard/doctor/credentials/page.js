...
export default function DoctorCredentials() {
  ...
  const btnBg = theme === "dark" ? "bg-emerald-700 hover:bg-emerald-800" : "bg-emerald-800 hover:bg-emerald-900";
  ...
  return (
    <div className={`max-w-4xl mx-auto space-y-6 ${bgMain} transition-colors duration-300`}>
      {/* Title */}
      <div>
        ...
        <p className={`text-sm ${textSecondary} mt-1 transition-colors duration-300`}>
          Configure your clinical specialities, qualifications, and billing options.
        </p>
      </div>

      <div className={`${bgCard} border ${cardBorder} rounded-2xl p-8 shadow-sm transition-colors duration-300`}>
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* Clinical Specialties */}
            <div>
              <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
                <Stethoscope className={`w-3.5 h-3.5 ${iconColor}`} /> Clinical Specialties
              </label>
              <select
                value={specialization}
                onChange={e => setSpecialization(e.target.value)}
            ...
            <div>
              <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
                <Award className={`w-3.5 h-3.5 ${iconColor}`} /> Experience (Years)
              </label>
              <input
                type="number"
                min="0"
            ...
            <div>
              <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
                <ShieldCheck className={`w-3.5 h-3.5 ${iconColor}`} /> Qualifications Statement
              </label>
              <input
                type="text"
                value={qualifications}
            ...
            <div>
              <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
                <DollarSign className={`w-3.5 h-3.5 ${iconColor}`} /> Co-Pay Consultation Fee ($)
              </label>
              <input
                type="number"
                min="0"
          ...
          <div>
            <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
              <Clock className={`w-3.5 h-3.5 ${iconColor}`} /> Attached Medical Hospital Name
            </label>
            <input
              type="text"
              value={hospitalName}
          ...
          <div>
            <label className={`block text-xs font-semibold ${labelText} uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors duration-300`}>
              <Image className={`w-3.5 h-3.5 ${iconColor}`} /> Profile Photo / Avatar Image (URL)
            </label>
            <input
              type="url"
              value={profileImage}
