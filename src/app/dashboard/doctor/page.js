...
export default function DoctorOverview() {
  ...
  const tagBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-gray-50 border-gray-200 text-gray-600";
  const avatarBg = theme === "dark" ? "bg-slate-800 border-slate-700" : "bg-gray-100 border-gray-200";
  const verificationBg = theme === "dark" ? "bg-amber-950/30 border-amber-800" : "bg-amber-50 border-amber-200";
  const verificationText = theme === "dark" ? "text-amber-400" : "text-amber-800";
  const verificationSubText = theme === "dark" ? "text-amber-400/70" : "text-amber-700";
  const statsIconBg = {
  ...
  if (!doctorProfile) {
    return (
      <div className={`max-w-2xl mx-auto py-8 ${bgMain} transition-colors duration-300`}>
        <div className={`${bgCard} border ${cardBorder} rounded-xl p-8 shadow-sm transition-colors duration-300`}>
          <h2 className={`text-2xl font-bold mb-2 text-center ${textPrimary} transition-colors duration-300`}>
            Doctor Onboarding
          </h2>
          <p className={`text-center text-xs ${textSecondary} mb-8 transition-colors duration-300`}>
            Complete your professional profile to register on MediCare Connect.
          </p>
          
          <form onSubmit={handleOnboard} className="space-y-6">
            <div>
              <label className={`block text-xs font-semibold ${textSecondary} uppercase tracking-wider mb-2 transition-colors duration-300`}>
                Qualifications
              </label>
              <input 
                type="text" 
  ...
  return (
    <div className={`space-y-8 ${bgMain} transition-colors duration-300`}>
      ...
      {/* Verification Banner */}
      {doctorProfile.verificationStatus === "pending" && (
        <div className={`${verificationBg} border rounded-xl p-4 flex gap-3 transition-colors duration-300`}>
          <Clock className={`w-5 h-5 ${verificationText} flex-shrink-0 mt-0.5`} />
          <div>
            <h3 className={`text-sm font-semibold ${verificationText}`}>Awaiting Verification</h3>
            <p className={`text-xs ${verificationSubText} mt-0.5 leading-relaxed`}>
              Your professional profile has been submitted and is currently pending verification by site administrators. Once verified, patients will see you in search lists.
            </p>
          </div>
        </div>
      ...
      {/* Stats Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className={`${bgCard} border ${cardBorder} ${cardHoverBorder} rounded-xl p-5 transition-all duration-300`}>
          <div className="flex items-start justify-between">
            <div>
            ...
            <div className={`w-9 h-9 rounded-lg ${statsIconBg.blue} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}>
        ...
        <div className={`${bgCard} border ${cardBorder} ${cardHoverBorder} rounded-xl p-5 transition-all duration-300`}>
          <div className="flex items-start justify-between">
            ...
            <div className={`w-9 h-9 rounded-lg ${statsIconBg.orange} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}>
        ...
        <div className={`${bgCard} border ${cardBorder} ${cardHoverBorder} rounded-xl p-5 transition-all duration-300`}>
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className={`mt-2 text-2xl font-semibold tracking-tight ${textPrimary} transition-colors duration-300`}>
                {doctorProfile.rating ? Number(doctorProfile.rating).toFixed(1) : "N/A"}
              </p>
            </div>
            <div className={`w-9 h-9 rounded-lg ${statsIconBg.yellow} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}>
              <Star className={`w-4 h-4 ${statsIconColor.yellow} fill-current`} />
            </div>
          </div>
        </div>
        <div className={`${bgCard} border ${cardBorder} ${cardHoverBorder} rounded-xl p-5 transition-all duration-300`}>
          <div className="flex items-start justify-between">
            ...
            <div className={`w-9 h-9 rounded-lg ${statsIconBg.green} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className={`${bgCard} border ${cardBorder} rounded-xl p-6 transition-colors duration-300`}>
          <h2 className={`text-base font-semibold ${textPrimary} mb-4 transition-colors duration-300`}>
            Profile Details
          </h2>
          <div className="space-y-4 text-sm">
            <div>
              <span className={`block text-xs font-medium ${textSecondary} uppercase tracking-wider transition-colors duration-300`}>
                Hospital
              </span>
              <p className={`text-sm font-semibold ${textPrimary} mt-0.5 transition-colors duration-300`}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1 mt-1.5">
                {doctorProfile.availableDays.map(d => (
                  <span key={d} className={`inline-flex ${tagBg} rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors duration-300`}>
                    {d}
                  </span>
                ))}
              </div>
        ...
        <div className={`${bgCard} border ${cardBorder} rounded-xl lg:col-span-2 overflow-hidden transition-colors duration-300`}>
          ...
          <div className={`divide-y ${divLight} max-h-80 overflow-y-auto`}>
            {pending.length === 0 ? (
              <p className={`text-sm ${textSecondary} p-8 text-center transition-colors duration-300`}>
                You're all caught up. No pending requests.
              </p>
            ) : (
              pending.slice(0, 5).map((a) => (
                <div key={a._id} className="p-4 flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-full ${avatarBg} flex items-center justify-center text-xs font-semibold flex-shrink-0 ${textMuted} transition-colors duration-300`}>
                    {(a.patientName || "P")[0]}
                  </div>
                  <div className="flex-1 min-w-0">
