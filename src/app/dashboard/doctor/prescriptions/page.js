...
function PrescriptionContent() {
  ...
  const deleteHover = theme === "dark" ? "hover:bg-red-950/30 hover:border-red-800" : "hover:bg-red-50 hover:border-red-200";
    ...
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      {prescriptions.length === 0 ? (
        <div className={`${bgCard} border ${cardBorder} rounded-xl p-12 text-center transition-colors duration-300`}>
          <ClipboardList className={`w-8 h-8 mx-auto ${emptyIcon} mb-3 transition-colors duration-300`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {prescriptions.map((pres) => (
            <div key={pres._id} className={`${bgCard} border ${cardBorder} rounded-xl p-5 flex flex-col justify-between transition-colors duration-300`}>
              <div>
                <div className="flex justify-between items-start gap-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-8 h-8 rounded-full ${avatarBg} flex items-center justify-center flex-shrink-0 transition-colors duration-300`}>
                      <User className={`w-4 h-4 ${avatarText}`} />
                    </div>
                    <div>
                      <p className={`text-xs font-medium ${textSecondary} transition-colors duration-300`}>Patient ID</p>
                      <p className={`text-sm font-semibold ${textPrimary} truncate max-w-[150px] transition-colors duration-300`}>
                        {pres.patientId}
                      </p>
                    </div>
                  </div>
                  <span className={`text-[10px] ${textMuted} whitespace-nowrap inline-flex items-center gap-1 transition-colors duration-300`}>
                    <Clock className="w-3 h-3" />
                    {new Date(pres.createdAt).toLocaleDateString()}
                  </span>
                </div>
                ...
                <div className="mt-4 space-y-2 text-sm">
                  <div className="flex items-start gap-1">
                    <span className={`font-semibold ${textPrimary} w-20 flex-shrink-0 transition-colors duration-300`}>Diagnosis:</span>
                    <span className={`${textBody} transition-colors duration-300`}>{pres.diagnosis}</span>
                  </div>
                  <div className="flex items-start gap-1">
                    <span className={`font-semibold ${textPrimary} w-20 flex-shrink-0 transition-colors duration-300`}>Meds:</span>
                    <span className={`${tagBg} border ${tagBorder} rounded px-1.5 py-0.5 text-xs font-mono ${tagText} transition-colors duration-300`}>
                      {pres.medications}
                    </span>
                  </div>
                  {pres.notes && (
                    <div className="flex items-start gap-1">
                      <span className={`font-semibold ${textPrimary} w-20 flex-shrink-0 transition-colors duration-300`}>Notes:</span>
                      <span className={`${textBody} italic transition-colors duration-300`}>"{pres.notes}"</span>
                    </div>
                  )}
                </div>
              </div>

              <div className={`mt-5 pt-3 border-t ${divLight} flex items-center justify-end gap-1.5 transition-colors duration-300`}>
                <button
                  onClick={() => handleEdit(pres)}
      ...
      {(addingPrescription || editingPrescription) && (
        ...
        >
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-4">
              <div>
                <label className={`block text-xs font-medium ${formLabel} mb-1.5 transition-colors duration-300`}>
                  Patient ID
                </label>
                <input
                  type="text"
