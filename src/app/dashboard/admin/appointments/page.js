...
export default function AdminAppointments() {
  ...
  const tableRowHover = theme === "dark" ? "hover:bg-slate-800/50" : "hover:bg-gray-50/50";
  ...
  const paymentPaid = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  const paymentRefunded = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-500" : "bg-gray-50 border-gray-200 text-gray-500";
  const paymentUnpaid = theme === "dark" ? "bg-blue-950/30 text-blue-400" : "bg-blue-50 text-blue-600";
    ...
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      <div>
        ...
        <p className={`text-sm ${textSecondary} mt-1 transition-colors duration-300`}>
          Oversee all clinical schedule bookings and statuses across the platform.
        </p>
      </div>
      ...
      <div className={`${bgCard} border ${cardBorder} rounded-xl overflow-hidden transition-colors duration-300`}>
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className={`text-sm font-semibold ${textPrimary} transition-colors duration-300`}>No appointments found</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className={`border-b ${divider} transition-colors duration-300`}>
                <tr className="text-left">
                  <th className={`px-5 py-3 text-xs font-medium ${tableHeader} uppercase tracking-wide transition-colors duration-300`}>
                    Patient & Doctor
                  </th>
                  <th className={`px-5 py-3 text-xs font-medium ${tableHeader} uppercase tracking-wide transition-colors duration-300`}>
                    Schedule Details
              ...
              <tbody className={`divide-y ${divLight}`}>
                {filtered.map((app) => (
                  <tr key={app._id} className={`${tableRowHover} transition-colors duration-200`}>
                    <td className="px-5 py-3.5">
                      <div className="space-y-1">
                        <p className={`text-sm font-semibold ${textPrimary} flex items-center gap-1.5 transition-colors duration-300`}>
                          <User className={`w-3.5 h-3.5 ${iconColor}`} />
                          {app.patientName}
                        </p>
                        <p className={`text-xs ${doctorText} flex items-center gap-1.5 transition-colors duration-300`}>
                          <Stethoscope className={`w-3.5 h-3.5 ${doctorIcon}`} />
                          Dr. {app.doctorName}
                        </p>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-xs">
                      <div className="space-y-1">
                        <span className={`inline-flex items-center gap-1 ${textPrimary} transition-colors duration-300`}>
                    ...
                    <td className={`px-5 py-3.5 text-xs ${textSecondary} max-w-xs truncate transition-colors duration-300`}>
                      {app.symptoms || "—"}
                    </td>
                    <td className="px-5 py-3.5">
                      <StatusPill status={app.appointmentStatus} theme={theme} />
                    </td>
                    <td className="px-5 py-3.5">
...
function PaymentPill({ status, theme }) {
  const normalized = (status || "").toLowerCase();
  const paidBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  const refundedBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-500" : "bg-gray-50 border-gray-200 text-gray-500";
  const unpaidBg = theme === "dark" ? "bg-blue-950/30 text-blue-400" : "bg-blue-50 text-blue-600";
  
  if (normalized === "paid") {
    return (
      <span className={`inline-flex items-center gap-1 ${paidBg} border rounded-full px-2.5 py-1 text-xs whitespace-nowrap transition-colors duration-300`}>
  ...
  if (normalized === "refunded") {
    return (
      <span className={`inline-flex items-center gap-1 ${refundedBg} border rounded-full px-2.5 py-1 text-xs whitespace-nowrap transition-colors duration-300`}>
        <span className="w-1.5 h-1.5 rounded-full bg-gray-400" />
        Refunded
      </span>
    );
  ...
  return (
    <span className={`inline-flex items-center gap-1 ${unpaidBg} rounded-full px-2.5 py-1 text-xs font-medium whitespace-nowrap transition-colors duration-300`}>
      Unpaid
    </span>
  );
}