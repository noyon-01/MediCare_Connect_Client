...
export default function AdminPayments() {
  ...
  const tagBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-600";
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className={`border-b ${divider} text-left transition-colors duration-300`}>
                  ...
                  <th className={`px-5 py-3 text-xs font-medium ${tableHeader} uppercase tracking-wide transition-colors duration-300 text-right`}>
                    Amount
                  </th>
                </tr>
              </thead>
              <tbody className={`divide-y ${divLight}`}>
                {payments.map((pay) => (
                  <tr key={pay._id} className={`${tableRowHover} transition-colors duration-200`}>
                    <td className="px-5 py-3.5">
                      <div className="space-y-1">
                    ...
                    <td className="px-5 py-3.5">
                      <PaymentPill status={pay.status || 'Paid'} theme={theme} />
                    </td>
                    <td className={`px-5 py-3.5 text-right font-bold ${textPrimary} transition-colors duration-300`}>
                      ${parseFloat(pay.amount || 0).toFixed(2)}
                    </td>
                  </tr>
                ))}
