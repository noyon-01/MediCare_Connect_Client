...
export default function AdminUsers() {
  ...
  const avatarBg = theme === "dark" ? "bg-blue-950/30 text-blue-400" : "bg-blue-50 text-blue-600";
  const roleBadge = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  const statusPill = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  const activateBtn = theme === "dark" ? "bg-slate-800 hover:bg-green-950/30 border-slate-700 hover:border-green-800 text-green-400" : "bg-white hover:bg-green-50 border-gray-200 hover:border-green-200 text-green-600";
  const suspendBtn = theme === "dark" ? "bg-slate-800 hover:bg-orange-950/30 border-slate-700 hover:border-orange-800 text-orange-400" : "bg-white hover:bg-orange-50 border-gray-200 hover:border-orange-200 text-orange-600";
  const deleteBtn = theme === "dark" ? "bg-slate-800 hover:bg-red-950/30 border-slate-700 hover:border-red-800 text-red-400" : "bg-white hover:bg-red-50 border-gray-200 hover:border-red-200 text-red-600";
  ...
  return (
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      {/* Filters Bar */}
      <div className={`${bgCard} border ${cardBorder} rounded-xl p-4 transition-colors duration-300`}>
        <div className="grid grid-cols-1 md:grid-cols-[1fr_200px] gap-3">
          <div className={`flex items-center gap-2 ${inputBg} border ${inputBorder} rounded-lg px-3 py-2 transition-colors duration-300`}>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody className={`divide-y ${divLight}`}>
                {filtered.map((u) => (
                  <tr key={u._id} className={`${tableRowHover} transition-colors duration-200`}>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2.5">
                        {u.image ? (
                          <img
                          ...
                          <div className={`w-8 h-8 rounded-full ${avatarBg} text-xs font-semibold flex items-center justify-center transition-colors duration-300`}>
                            {(u.name || u.email || "U")[0].toUpperCase()}
                          </div>
                        )}
                        <div>
                          ...
                          <p className={`text-xs ${textSecondary} transition-colors duration-300`}>{u.email}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center ${roleBadge} border rounded-full px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide transition-colors duration-300`}>
                        {u.role || "patient"}
                      </span>
                    </td>
                    <td className={`px-5 py-3.5 text-xs ${textSecondary} transition-colors duration-300`}>
                      <p>{u.phone || "No Phone"}</p>
                      <p className={`${textMuted} capitalize text-[10px] mt-0.5 transition-colors duration-300`}>{u.gender || "—"}</p>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center gap-1.5 ${statusPill} border rounded-full px-2.5 py-0.5 text-xs transition-colors duration-300`}>
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${
                            u.status === "suspended"
                              ? "bg-red-500"
