...
export default function AdminDoctors() {
  ...
  const verifyBtn = theme === "dark" ? "bg-blue-950/30 hover:bg-blue-950/50 text-blue-400" : "bg-blue-50 hover:bg-blue-100 text-blue-600";
  ...
  const revokeBtn = theme === "dark" ? "bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-300" : "bg-white hover:bg-gray-50 border-gray-200 text-gray-700";
  const statusPillBg = theme === "dark" ? "bg-slate-800 border-slate-700 text-slate-300" : "bg-white border-gray-200 text-gray-700";
  const emptyBg = theme === "dark" ? "bg-slate-900 border-slate-800" : "bg-white border-gray-200";
    ...
    <div className={`space-y-6 ${bgMain} transition-colors duration-300`}>
      ...
      <div>
        ...
        <p className={`text-sm ${textSecondary} mt-1 transition-colors duration-300`}>
          Verify doctor credentials, reject verification requests, or cancel verified status.
        </p>
      </div>
      ...
      {/* Grid */}
      {filtered.length === 0 ? (
        <div className={`${emptyBg} border rounded-xl p-12 text-center text-sm transition-colors duration-300`}>
          <p className={textSecondary}>No doctor profiles found.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((d) => (
            <div key={d._id} className={`${bgCard} border ${cardBorder} rounded-xl p-5 flex flex-col justify-between transition-colors duration-300`}>
              <div>
                <div className="flex items-start gap-3">
                  <div className={`w-12 h-12 rounded-lg ${avatarBg} overflow-hidden flex-shrink-0 flex items-center justify-center transition-colors duration-300`}>
                    {d.profileImage ? (
                      <img src={d.profileImage} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <div className={`w-full h-full flex items-center justify-center ${avatarText} text-sm font-semibold`}>
                        {(d.doctorName || "D")[0]}
                      </div>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <span className={`inline-flex items-center gap-1 ${tagBg} border rounded-full px-2 py-0.5 text-[11px] transition-colors duration-300`}>
                        {d.experience}+ yrs exp
                      </span>
                      <span className={`inline-flex items-center gap-1 ${tagBg} border rounded-full px-2 py-0.5 text-[11px] transition-colors duration-300`}>
                        ${d.consultationFee} fee
                    ...
                    <p className={`text-xs ${textMuted} mt-2 truncate transition-colors duration-300`}>
              ...
              <div className={`mt-4 pt-4 border-t ${divLight} flex gap-2 justify-end transition-colors duration-300`}>
                {d.verificationStatus !== "verified" && (
                  <button
                    onClick={() => updateVerification(d._id, "verified")}
...
function StatusPill({ status, theme }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 ${bg} border rounded-full px-2.5 py-0.5 text-[11px] whitespace-nowrap transition-colors duration-300`}>
      <span className={`w-1.5 h-1.5 rounded-full ${s.dot}`} />
      {s.label}
    </span>
  );
