...
function BookingForm({ doctor, session }) {
  ...
  if (isNotPatient) {
    const roleLabel =
      session.user.role === "doctor" ? "Doctor" : "Administrator";
    return (
      <div
        className={`${cardBg} border ${cardBorder} rounded-xl p-6 space-y-4 transition-colors duration-300`}
      >
        <div>
          ...
          <p className={`text-xs ${textSecondary} mt-1`}>
            Bookings are reserved for patients only.
          </p>
        </div>
        <div
          className={`${roleBg} border rounded-xl p-4 text-xs leading-relaxed transition-colors duration-300`}
        >
          <p className="font-semibold mb-1">Authenticated as {roleLabel}</p>
          Only patients can schedule consultations and pay fees. To schedule an
          appointment, please log out and sign in using a Patient account.
        </div>
      </div>
    );
  }
  ...
  const handleBook = async (e) => {
    ...
    try {
      ...
      const dayStr = matchDate
        ? `${matchDate.fullName} (${selectedDate})`
        : selectedDate;
  ...
  return (
    <form
      onSubmit={handleBook}
      className={`${cardBg} border ${cardBorder} rounded-xl p-6 space-y-5 transition-colors duration-300`}
    >
      <div>
        <h3 className={`text-base font-semibold ${textPrimary}`}>
          Book an appointment
        ...
        <p className={`text-xs ${textSecondary} mt-1`}>
          Pay securely with Stripe Checkout to confirm.
        </p>
      </div>

      <div>
        <label
          className={`block text-xs font-medium ${textSecondary} mb-2 flex items-center gap-1`}
        >
        ...
        <div className="flex gap-1.5 overflow-x-auto pb-1">
          {nextDates.map((d) => (
            ...
            >
              <p className={`text-[10px] font-medium uppercase ${textMuted}`}>
                {d.dow}
              </p>
              <p className={`text-base font-semibold ${textPrimary} my-0.5`}>
                {d.day}
              </p>
      ...
      <div
        className={`pt-4 border-t ${divider} flex items-center justify-between`}
      >
        <div>
...
export default function DoctorDetails() {
  ...
  if (loading) {
    return (
      <div
        className={`flex justify-center items-center min-h-[50vh] ${bgMain}`}
      >
  ...
  if (!doctor) {
    return (
      <div className={`container mx-auto px-4 py-20 text-center ${bgMain}`}>
        ...
        <a href="/doctors" className="text-blue-600 text-sm mt-4 inline-block">
          ← Back to doctors
        </a>
      </div>
    );
  ...
  return (
    <div className={`min-h-screen ${bgMain} transition-colors duration-300`}>
      {/* Header */}
      <div
        className={`border-b ${borderColor} ${bgHeader} transition-colors duration-300`}
      >
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            >
              <div className="flex flex-col sm:flex-row gap-6">
                <div
                  className={`w-28 h-28 rounded-xl ${avatarBg} border ${avatarBorder} overflow-hidden flex-shrink-0 transition-colors duration-300`}
                >
                  {doctor.profileImage ? (
                    <img
                    ...
                    <div
                      className={`w-full h-full flex items-center justify-center ${textMuted} text-3xl font-semibold ${avatarBg}`}
                    >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    {doctor.verificationStatus === "verified" && (
                      <span
                        className={`inline-flex items-center gap-1 ${verifiedBg} border rounded-full px-2.5 py-1 text-xs transition-colors duration-300`}
                      >
                  ...
                  <div className="mt-4 flex flex-wrap gap-1.5">
                    <span
                      className={`inline-flex items-center gap-1 ${tagBg} border ${tagBorder} rounded-full px-2.5 py-1 text-xs ${tagText} transition-colors duration-300`}
                    >
              ...
              {doctor.qualifications && (
                <div className={`mt-6 pt-6 border-t ${divider}`}>
                  <h3
                    className={`text-sm font-semibold ${textPrimary} mb-2 flex items-center gap-1.5`}
                  >
              ...
              <div className={`mt-6 pt-6 border-t ${divider}`}>
                <h3
                  className={`text-sm font-semibold ${textPrimary} mb-3 flex items-center gap-1.5`}
                >
            ...
            >
              ...
              {reviews.length === 0 ? (
                <p
                  className={`text-sm ${textSecondary} py-6 text-center italic`}
                >
                  No reviews yet for this doctor. Be the first to leave feedback
                  after booking!
                </p>
              ) : (
                <div className="space-y-4">
                  {reviews.map((r) => (
                    ...
                    >
                      ...
                      <p
                        className={`text-sm ${textSecondary} leading-relaxed mt-2 pl-9`}
                      >
                        "{r.reviewText}"
                      </p>
                    </div>
