...
export default function DashboardSidebar({ user, role, isVerifiedDoctor }) {
  ...
  const sidebarBorder = theme === "dark" ? "border-slate-800" : "border-gray-200";
  ...
  const mobileBarBorder = theme === "dark" ? "border-slate-800" : "border-gray-200";
  const navActive = theme === "dark" 
    ? "bg-blue-950/30 text-blue-400" 
    : "bg-blue-50 text-blue-600";
  const navHover = theme === "dark" 
    ? "hover:bg-slate-800" 
    : "hover:bg-gray-50";
  const navText = theme === "dark" 
    ? "text-slate-300 hover:text-white" 
    : "text-gray-700 hover:text-gray-900";
  const roleBadge = theme === "dark" 
  ...
  const logoutHover = theme === "dark" ? "hover:bg-red-950/30" : "hover:bg-red-50";
  const backHover = theme === "dark" ? "hover:bg-slate-800 hover:text-white" : "hover:bg-gray-50 hover:text-gray-900";
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className={`lg:hidden sticky top-0 z-30 ${mobileBarBg} border-b ${mobileBarBorder} px-4 h-14 flex items-center justify-between w-full flex-shrink-0 transition-colors duration-300`}>
        <a href="/">
          <Logo size="sm" />
        </a>
        <div className="flex items-center gap-1.5">
      ...
      >
        {/* Logo */}
        <div className={`h-16 flex items-center px-5 border-b ${sidebarBorder} flex-shrink-0 transition-colors duration-300`}>
          <a href="/">
            <Logo />
          </a>
        </div>
        ...
        {/* User Info */}
        <div className={`p-3 border-b ${sidebarBorder} flex-shrink-0 transition-colors duration-300`}>
          <div className="flex items-center gap-2.5">
            {user.image && !imgError ? (
              <img
                src={user.image}
            ...
            <div className="min-w-0 flex-1">
              <p className={`text-xs font-semibold ${textPrimary} truncate transition-colors duration-300`}>
                {user.name || "Account"}
              </p>
              <p className={`text-[10px] ${textSecondary} truncate transition-colors duration-300`}>
                {user.email}
              </p>
            </div>
        ...
        {/* Bottom Actions */}
        <div className={`p-3 border-t ${sidebarBorder} flex-shrink-0 space-y-0.5 transition-colors duration-300`}>
          <div className={`flex items-center justify-between px-3 py-1 mb-1.5 ${themeToggleBg} rounded-lg transition-colors duration-300`}>
            <span className={`text-xs ${textSecondary} font-medium transition-colors duration-300`}>
              Dark Mode
            </span>
            <ThemeToggle />
          </div>
