...
export default function DoctorCard({ doctor, index = 0 }) {
  ...
  const themeClasses = {
    ...
    bookShadow: theme === "dark" 
      ? "group-hover:shadow-blue-900/30" 
      : "group-hover:shadow-blue-200",
    avatarBorder: theme === "dark" ? "border-slate-700" : "border-slate-200",
    avatarHoverBorder: theme === "dark" ? "group-hover:border-slate-600" : "group-hover:border-blue-400",
    avatarBg: theme === "dark" ? "from-slate-800 to-slate-700" : "from-slate-100 to-slate-200",
      ...
      <div className="p-5">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="relative flex-shrink-0">
            <div className={`w-16 h-16 rounded-2xl overflow-hidden bg-gradient-to-br ${themeClasses.avatarBg} border-2 ${themeClasses.avatarBorder} ${themeClasses.avatarHoverBorder} transition-colors duration-300`}>
              {doctor.profileImage ? (
                <img
                  src={doctor.profileImage}
                  alt={doctor.doctorName}
                ...
                <div className={`w-full h-full flex items-center justify-center ${themeClasses.textMuted} font-bold text-xl bg-gradient-to-br from-blue-50 to-purple-50 dark:from-slate-800 dark:to-slate-700`}>
                  {(doctor.doctorName || "D")[0]}
                </div>
              )}
            </div>
            {/* Online Status */}
            {isAvailable && (
              <span className={`absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 border-2 ${themeClasses.statusBorder} rounded-full flex items-center justify-center`}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <h3 className={`text-base font-bold ${themeClasses.textPrimary} truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300`}>
                  {doctor.doctorName}
                </h3>
                <p className={`text-xs ${themeClasses.textSecondary} mt-0.5 flex items-center gap-1.5`}>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <span className={`inline-flex items-center gap-1.5 ${themeClasses.tagBg} border ${themeClasses.tagBorder} rounded-full px-3 py-1.5 text-xs ${themeClasses.tagText}`}>
            <Briefcase className={`w-3 h-3 ${themeClasses.textMuted}`} />
            {doctor.experience || 0} Years
          </span>
          <span className={`inline-flex items-center gap-1.5 ${themeClasses.tagBg} border ${themeClasses.tagBorder} rounded-full px-3 py-1.5 text-xs ${themeClasses.tagText} truncate max-w-[180px]`}>
            <MapPin className={`w-3 h-3 ${themeClasses.textMuted} flex-shrink-0`} />
        ...
        {/* Bottom - Fee & Action */}
        <div className={`mt-4 pt-4 border-t ${themeClasses.divider} flex items-center justify-between`}>
          <div>
            <p className={`text-[10px] ${themeClasses.textMuted} uppercase tracking-wider font-semibold`}>
              Consultation Fee
            </p>
            <div className="flex items-baseline gap-1.5">
              ...
              <span className={`text-[10px] ${themeClasses.textMuted} line-through`}>
          ...
          <div className="flex items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 ${themeClasses.bookBtn} rounded-full px-4 py-2 text-xs font-semibold transition-all duration-300 group-hover:shadow-lg ${themeClasses.bookShadow}`}>
        ...
        {/* Hover Overlay Glow */}
        <div className={`absolute -inset-full -z-10 bg-gradient-to-r ${themeClasses.glowBg} opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-2xl`} />
      </div>

      {/* Rating Stars Progress Indicator */}
      <div className={`absolute bottom-0 left-0 right-0 h-0.5 ${themeClasses.progressBg}`}>
        <motion.div
          initial={{ width: 0 }}
